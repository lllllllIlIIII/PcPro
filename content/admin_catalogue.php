...
$compDAO = new ComposantDAO($pdo); // NOUVEAU : On appelle la boîte à outils des composants
$message = "";
...
// --- PRÉPARATION DE L'HISTORIQUE UNIFIÉ ---
$catalogue_orders = $catalogueDAO->getCatalogueOrders();
$custom_orders = $catalogueDAO->getCustomOrders();
...
<div class="container mt-5 mb-5">
    ...
    <div class="card shadow-sm border-light bg-light mb-5">
        <div class="card-body p-4">
            ...
            <form action="index.php?page=admin_catalogue" method="POST">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="cpu" class="form-select form-select-sm" required>
                            ...
                            <?php foreach($compDAO->getComposantsByCategorie(1) as $c): ?>
                                <?php $selected = ($pc_a_modifier && $pc_a_modifier->getProcesseur() === $c->getNom()) ? 'selected' : ''; ?>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="gpu" class="form-select form-select-sm" required>
                            ...
                            <?php foreach($compDAO->getComposantsByCategorie(2) as $c): ?>
                                <?php $selected = ($pc_a_modifier && $pc_a_modifier->getCarteGraphique() === $c->getNom()) ? 'selected' : ''; ?>
                                <option value="<?= htmlspecialchars($c->getNom()) ?>" <?= $selected ?>><?= htmlspecialchars($c->getNom()) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="mb" class="form-select form-select-sm" required>
                            ...
                            <?php foreach($compDAO->getComposantsByCategorie(4) as $c): ?>
                                <?php $selected = ($pc_a_modifier && $pc_a_modifier->getCarteMere() === $c->getNom()) ? 'selected' : ''; ?>
                                <option value="<?= htmlspecialchars($c->getNom()) ?>" <?= $selected ?>><?= htmlspecialchars($c->getNom()) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="ram" class="form-select form-select-sm" required>
                            ...
                            <?php foreach($compDAO->getComposantsByCategorie(3) as $c): ?>
                                <?php $selected = ($pc_a_modifier && $pc_a_modifier->getMemoire() === $c->getNom()) ? 'selected' : ''; ?>
                                <option value="<?= htmlspecialchars($c->getNom()) ?>" <?= $selected ?>><?= htmlspecialchars($c->getNom()) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="stock" class="form-select form-select-sm" required>
                            ...
                            <?php foreach($compDAO->getComposantsByCategorie(5) as $c): ?>
                                <?php $selected = ($pc_a_modifier && $pc_a_modifier->getStockage() === $c->getNom()) ? 'selected' : ''; ?>
                                <option value="<?= htmlspecialchars($c->getNom()) ?>" <?= $selected ?>><?= htmlspecialchars($c->getNom()) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                ...
                <button type="submit" name="btn_valider" class="btn btn-dark fw-bold px-4 mt-2">
        ...
        <div class="card-body p-0">
            <table class="table table-hover align-middle mb-0">
                ...
                <?php foreach ($pcs as $pc): ?>
                    <tr>
                        <td class="px-4">
                            <img src="admin/assets/img/<?php echo htmlspecialchars($pc->getImageUrl()); ?>" style="width: 50px; height: 50px; object-fit: contain;">
                        </td>
    ...
    <div class="d-flex justify-content-between align-items-center mt-5 mb-4">
        ...
        <div class="btn-group shadow-sm" role="group">
            <button type="button" class="btn btn-outline-info active btn-filter" data-filter="all" style="border-radius: 0;">Tout afficher</button>
            <button type="button" class="btn btn-outline-info btn-filter" data-filter="catalogue" style="border-radius: 0;">Catalogue uniquement</button>
            <button type="button" class="btn btn-outline-info btn-filter" data-filter="custom" style="border-radius: 0;">Sur Mesure uniquement</button>
        </div>
    </div>

    <div class="card shadow-sm border-light mb-5" style="border-radius: 0;">
        <div class="card-body p-0">
                    ...
                    <?php foreach ($historique_global as $order): ?>
                        <tr class="row-order" data-type="<?php echo $order['type']; ?>">
                            <td class="px-4">
                                ...
                                <?php else: ?>
                                    <span class="badge" style="background-color: var(--tech-primary, #66fcf1); color: #000;">CUSTOM</span>
                                <?php endif; ?>
                            </td>
                            ...
                            <td>
                                <small class="d-block text-light" style="white-space: pre-line; line-height: 1.3;">
